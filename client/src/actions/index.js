...
//         payload: json.data   
//     })}}; 
export function getPokemons(){  // esta Action permite renderizar todos los Games
    return function(dispatch){
...
export function getNamePokemons(name){ // esta Action permite BUSQUEDA todos los Games por nombres
...
export function getListTypes(){ //(GameCreate) (HOME) Me trae los Generos
    return function(dispatch){
        ...
        .catch(()=>{ alert('Error al traer Tipos')})
    }
}
// PARA EL POST DE GENRES y CREAR AL NUEVO JUEGO
export function postPokemon(payload){ //recibe un objeto con toda la info del Game a crear (GameCreate)
    return async function (dispatch){
        const response = await axios.post("http://localhost:3001/newPokemons", payload)
...
export function getDetailPokemons(id){
    return function(dispatch){
        ...
        .then((response)=>{
            dispatch({type:'GET_DETAILS_POKEMONS', payload: response.data})
        })
        .catch(()=>{
